# -*- coding: utf-8 -*-
"""
live_trade_engine_v31_14_failsafe.py

V31_14_Failsafe · 多币轮动 · Binance USDT-M 永续合约 实盘引擎

特性：
1）继承 V31_13 全部交易逻辑（信号 / 开仓 / 平仓 / 风控 不变）
2）启动时自动接管交易所已有仓位（和 V31_13 一样）
3）新增 Failsafe：
...
# ==============================
# 多币轮动实盘引擎 · V31_14_Failsafe
# ==============================
class LiveTradeEngineV31_14:
    def __init__(
        self,
    ...
    def _failsafe_force_close(self, sym: str, pos: LivePositionV31):
        ...
        for p in positions:
            ...
            if self.binance.hedge_mode:
                ...
                # 单向模式：只有一个 BOTH 仓位，用正负判断方向
                real_side = "LONG" if pos_amt > 0 else "SHORT"
                real_qty = abs(pos_amt)
                break
        ...
        for sym, pos in list(self.positions.items()):
            ...
            if self.cfg_live.live_mode and self.binance is not None and pos.qty > 0:
                ...
                if order is not None:
                    ...
                    # Failsafe 里如果成功，会自己 pop 掉仓位，这里就不再加入 to_close
            else:
                # 非实盘模式 / 无 qty，仅在模型层平仓
                to_close.append(sym)
    ...
    def run_one_cycle(self):
        ...
        print(
            f"\n==== V31_14_Failsafe 周期刷新 · symbols={self.cfg_live.symbols}, "
    ...
    def run_forever(self):
        print("==== 启动 V31_14_Failsafe 多币轮动·Binance USDT-M Futures 实盘引擎（实时循环） ====")
        print(
...
# ===========================
def parse_args():
    p = argparse.ArgumentParser(description="V31_14_Failsafe 多币轮动 · Binance USDT-M Futures 实盘引擎")
    p.add_argument(
        "--symbols",
        type=str,
...
def main():
    ...
    engine = LiveTradeEngineV31_14(cfg_live, binance=binance_client)
    engine.run_forever()
