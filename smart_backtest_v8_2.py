    ...
    def run_symbol_backtest(
        ...
        for idx, row in d.iterrows():
            ...
            if position == 0:
                ...
                # 趋势 & RSI（改为“稍宽松”）
                trend_up = bool(row["trend_long_ok"])
                trend_down = bool(row["trend_short_ok"])
                rsi_long_ok = bool(row["rsi_long_ok"])
                rsi_short_ok = bool(row["rsi_short_ok"])
                ...
                # 方向合成：趋势 OR RSI（而非必须同时为真）
                long_signal = dir_long and (trend_up or rsi_long_ok)
                short_signal = dir_short and (trend_down or rsi_short_ok)
                ...
                # Entry Score 打分：趋势 + 策略共识（更宽松）
                consensus_strength = min(1.0, abs(ensemble_raw * 2.0))
                entry_score = 0.3 * trend_strength + 0.7 * consensus_strength
                if entry_score < 0.4:  # 从 0.6 降到 0.4
                    continue
...
def run_backtest(
    ...
    logger.info("🚀 SmartBacktest V8_2 启动")
...
def main():
    parser = argparse.ArgumentParser(description="SmartBacktest V8_2")
    parser.add_argument(
        "--symbols",
        type=str,
