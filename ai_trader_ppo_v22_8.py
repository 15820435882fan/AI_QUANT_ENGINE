"""
ai_trader_ppo_v22_8.py

V22_8: AI Trader · PPO 训练器（Train / Valid / Test 三阶段）
---------------------------------------------------------

用法示例：
    python ai_trader_ppo_v22_8.py --symbol BTCUSDT --days 365 --total-steps 100000

仅评估模式：
    python ai_trader_ppo_v22_8.py --symbol BTCUSDT --days 365 --mode eval \
        --model-path models/ai_trader_ppo_V22_8_BTCUSDT_best_valid.pt --eval-split test
"""
...
from trading_env_v22_8 import TradingEnvV22MultiTFV8
...
def train_ppo(
    ...
    # 三段环境（V22_8 支持 split_mode）
    env_train = TradingEnvV22MultiTFV8(symbol=symbol, days=days, segment="train", split_mode=split_mode)
    env_valid = TradingEnvV22MultiTFV8(symbol=symbol, days=days, segment="valid", split_mode=split_mode)
    env_test = TradingEnvV22MultiTFV8(symbol=symbol, days=days, segment="test", split_mode=split_mode)
    ...
    env_test = TradingEnvV22MultiTFV8(symbol=symbol, days=days, segment="test")
...
def evaluate_model(
    ...
    env = TradingEnvV22MultiTFV8(symbol=symbol, days=days, segment=eval_split, split_mode=split_mode)
