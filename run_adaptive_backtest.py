...
import pandas as pd
import numpy as np
from typing import Dict, List, Optional, Any  # 🔧 添加类型导入
...
class AdaptiveBacktestEngine:
    ...
    async def run_adaptive_backtest(self, historical_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # 滑动窗口分析
        window_size = 1440  # 1天的数据（1440分钟）
        
        for i in range(window_size, len(historical_data), 60):  # 每小时检测一次
...
async def main():
    ...
    print(f"🔄 市场状态变化: {result['regime_changes']} 次")
    
    print(f"\n📊 策略表现:")
    for strategy, perf in result['strategy_performance'].items():
        win_rate = (perf['win_trades'] / perf['trades']) if perf['trades'] > 0 else 0
        avg_profit = (perf['total_profit'] / perf['trades']) if perf['trades'] > 0 else 0
        print(f"  {strategy}: {perf['trades']}次交易, 胜率: {win_rate:.1%}, 平均利润: {avg_profit:.2f}")
    
    # 显示市场状态历史
    if result['regime_history']:
        regimes = [r['regime'] for r in result['regime_history']]
        print(f"\n📈 市场状态分布:")
