...
class MACDStrategyOptimized(BaseStrategy):
    ...
    def _check_buy_condition(self, current_macd: float, current_signal: float, 
        ...
        macd_near_cross = (current_macd <= current_signal and 
                          abs(current_macd - current_signal) < abs(current_macd) * 0.1)
        
        # 条件2: 柱状图改善趋势
        hist_improving = current_histogram > prev_histogram
        hist_positive_turn = prev_histogram <= 0 and current_histogram > 0
        hist_strong_improve = (prev_histogram < 0 and current_histogram > 0 and 
                              abs(current_histogram - prev_histogram) > self.hist_threshold)
        
        # 条件3: 柱状图连续改善
        hist_trend_improving = (current_histogram > prev_histogram > prev2_histogram)
        
        # 条件4: 有一定的趋势强度
        has_trend = trend_strength > self.min_trend_strength
        
        # 综合条件 - 更灵活的组合
        if ((macd_cross_up or macd_near_cross) and 
            (hist_strong_improve or (hist_improving and has_trend))):
            
            reason_parts = []
            if macd_cross_up:
                reason_parts.append("MACD金叉")
            elif macd_near_cross:
                reason_parts.append("MACD接近金叉")
                
            if hist_strong_improve:
                reason_parts.append(f"Hist强势转正({prev_histogram:.4f}→{current_histogram:.4f})")
            elif hist_improving:
                reason_parts.append(f"Hist改善趋势")
                
            if has_trend:
                reason_parts.append(f"趋势确认({trend_strength:.4f})")
