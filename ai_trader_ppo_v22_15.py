"""
ai_trader_ppo_v22_12.py

V22_15: AI Trader · PPO 训练器（Train / Valid / Test 三阶段）
---------------------------------------------------------

用法示例：
    python ai_trader_ppo_v22_12.py --symbol BTCUSDT --days 365 --total-steps 100000

仅评估模式：
    python ai_trader_ppo_v22_12.py --symbol BTCUSDT --days 365 --mode eval \
        --model-path models/ai_trader_ppo_V22_15_BTCUSDT_best_valid.pt --eval-split test
"""
...
from trading_env_v22_15 import TradingEnvV22MultiTFV15
...
def train_ppo(
    ...
    # 三段环境
    env_train = TradingEnvV22MultiTFV15(symbol=symbol, days=days, segment="train")
    env_valid = TradingEnvV22MultiTFV15(symbol=symbol, days=days, segment="valid")
    env_test = TradingEnvV22MultiTFV15(symbol=symbol, days=days, segment="test")
    ...
    while steps_collected < total_steps:
        ...
        if train_round % config.valid_eval_interval == 0:
            ...
            if valid_equity > best_valid_equity:
                ...
                logger.info("[VALID] 验证未提升，继续训练（不早停，仅更新 best_valid）")
...
def evaluate_model(
    ...
    env = TradingEnvV22MultiTFV15(symbol=symbol, days=days, segment=eval_split)  # 修复类名
