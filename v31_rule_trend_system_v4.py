# v31_rule_trend_system_v4.py
# V31 · 规则版 Teacher_V4
# 专业版：4H/1H 大趋势 + 15m 趋势单执行 + 5m 右侧反转单 + ATR 动态止损 + Excel 导出
#
# 用户已确认配置：
#   1）趋势单执行周期 = 15m
#   2）反转单使用 StochRSI 右侧拐点，不强制 K 线右侧形态
#
...
class V31RuleTrendSystemV4:
    ...
    def _entry_signal(
        ...
        # ---------- 2）15m 趋势信号（仅在非冲突区 + 15m bar 执行） ----------
        if not conflict_zone and macro_dir != 0 and trend_dir != 0 and is_15m_bar:
            # 多头趋势
    ...
    def summary(self):
        ...
        print("===== V31 规则版 Teacher_V4 · 趋势15m + 5m右侧反转 回测结果 =====")
    ...
    def _plot_equity(self):
        ...
        plt.title(f"V31 规则版 Teacher_V4 资金曲线 (5m执行, {self.cfg.symbol})")
        plt.xlabel("Time")
...
def parse_args():
    p = argparse.ArgumentParser(description="V31 规则版 Teacher_V4 · 趋势15m + 5m右侧反转 回测")
...
if __name__ == "__main__":
    ...
    system = V31RuleTrendSystemV4(cfg)
