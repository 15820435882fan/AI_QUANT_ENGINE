...
#
# V22_17: Multi-timeframe trading environment (strong trend window version)
# - Based on V22_16:
#   * Keep Reward Shaping 4.0 (trend x3, momentum x2, stronger anti-trend and drawdown penalties)
#   * Change z-score window from 200 -> 100 to reduce warm-up and increase usable_len
#   * Class renamed to TradingEnvV22MultiTFV17 and log prefix [EnvV22_17]
...
TRADING_FEE_RATE = 0.0005  # one-way fee
SLIPPAGE_RATE = 0.0002     # simple slippage

try:
    from local_data_engine import load_local_kline
except Exception:
    def load_local_kline(*args, **kwargs):
        raise RuntimeError("local_data_engine.load_local_kline not found. Make sure the file exists in the same directory.")
...
class Position:
    ...
    leverage: float
    entry_price: float
    size: float      # position size (contracts, notional = price * size)


def _ensure_ohlc(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure OHLC columns and sorted datetime index."""
    df = df.copy()
...
def _clean_ohlc(df: pd.DataFrame) -> pd.DataFrame:
    """Basic OHLC cleaning: deduplicate, sort, remove extreme candles."""
    df = df.copy()
    ...
    # Remove extremely large return outliers via z-score (> 10 sigma)
...
def _zscore(series: pd.Series, window: int = 100) -> pd.Series:
    """Rolling z-score with default window=100 (V22_17 adjustment)."""
...
class TradingEnvV22MultiTFV17:
    """RL trading environment (single symbol, multi-timeframe state, Reward Shaping 4.0 + data split).

    segment:
        - "train": use first train_ratio of data
        - "valid": use middle valid_ratio of data
        - "test" : use remaining data
    """
    ...
    def __init__(
        ...
        assert segment in ("train", "valid", "test"), f"Invalid segment: {segment}"
        ...
        # aligned state table and current segment subset
        ...
        logger.info(
            f"[EnvV22_17] segment={self.segment}, usable_len={len(self.df_state)}, "
            f"state_dim={self.state_dim}"
        )

    def _load_data_multitf(self) -> None:
        """Load multi-timeframe data."""
        logger.info(f"[EnvV22_17] Loading data: {self.symbol}, days={self.days}")
    ...
    def _build_multitf_state(self) -> None:
        """Build multi-timeframe state features."""
        df1 = self.df_1h.copy()
        close_1h = df1["close"]
        ...
        # regime classification with z-score window=100
        trend_proxy = (merged_4h_15["ema_fast_ratio"] - merged_4h_15["ema_slow_ratio"]).abs()
        vol_proxy = merged_4h_15["vol_24"].fillna(0)

        t_norm = _zscore(trend_proxy.fillna(0)).clip(-3, 3)
        v_norm = _zscore(vol_proxy).clip(-3, 3)
    ...
    def _apply_segment_split(self) -> None:
        """Split df_state_full into train/valid/test according to ratios and segment."""
        df = self.df_state_full
        n = len(df)
        if n < 100:
            raise ValueError(f"Too few state rows: {n}")
        ...
        if len(seg_df) < 50:
            raise ValueError(f"segment={self.segment} too short: {len(seg_df)}")
...
if __name__ == "__main__":
    env = TradingEnvV22MultiTFV17(symbol="BTCUSDT", days=500, segment="train")
    obs = env.reset()
    logger.info(f"[EnvV22_17] init obs shape: {obs.shape}, first 5: {obs[:5]}")
    total_r = 0.0
    steps = 0
    done = False
    while not done and steps < 300:
        ...
        steps += 1
    logger.info(
        f"[EnvV22_17] random policy test: steps={steps}, total_reward={total_r:.4f}, final_equity={info.get('equity', None):.2f}"
    )
