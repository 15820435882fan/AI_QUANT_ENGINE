...
class V31_1_Config:
    ...
    leverage: float = 5.0
    pos_cap_strong: float = 2.0
    pos_cap_normal: float = 1.4
    ...
    # 持仓时间（bar 数）
    max_bars_strong: int = 200
    max_bars_normal: int = 100
            ...
            if side == "FLAT":
                ...
                if sig_side != "FLAT" and sig_type is not None:
                    ...
                    if atr_1h > 0 and self.account.risk_capital() > 0:
                        ...
                        # 用总权益做仓位计算（交易资金 + 利润池一起参与复利）
                        risk_equity = self.account.total_equity()
