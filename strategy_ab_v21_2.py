"""
strategy_ab_v21_2.py

V21_2: 多周期趋势 + 独立多因子系统
--------------------------------
- 策略 A：多周期趋势跟随（1h 主趋势 + 15m 启动，4h 作为软权重），频次较 V21 明显提升
- 策略 B：纯多因子 AI 策略（不依赖 4h/1h 趋势过滤，独立判断、独立交易）

共同特性：
- 使用本地 K 线：local_data_engine.load_local_kline(symbol, interval, days)
- 执行周期：1h（所有决策在 1h 上进行）
- 多空、3~10 倍杠杆（由置信度线性映射）
- ATR 止损 + 趋势型追踪止盈（A） / 较紧 ATR 风控（B）
- 冷静期 + 最小持仓 bars，控制交易频率
- 每笔交易记录保存为 CSV 到 ./testdata 目录下：
    V21_2_A_BTCUSDT_001.csv
    V21_2_B_BTCUSDT_001.csv
  可直接用 Excel 打开

用法示例（在项目根目录执行）：
    python strategy_ab_v21_2.py --symbols BTCUSDT,ETHUSDT --days 365
"""
...
def next_sequence_number(symbol: str, strategy_tag: str) -> str:
    """
    扫描 OUTPUT_DIR 下已有的 V21_2_<strategy>_<symbol>_XXX.csv
...
def build_4h_trend(df_4h: pd.DataFrame) -> pd.DataFrame:
    """4h 级别趋势（软因子，用于 A 的置信度和方向修正，不再硬过滤）。"""
    df = df_4h.copy()
    close = df["close"]
...
def build_15m_breakout(df_15m: pd.DataFrame, window: int = 20) -> pd.DataFrame:
    ...
    然后聚合到 1h 层（按 floor('H')），只要当小时内出现过就认为触发。
    """
    df = df_15m.copy()
    high = df["high"]
    low = df["low"]
...
def build_signals_A(df_1h: pd.DataFrame,
    ...
    """
    策略 A（V21_2）：
      - 核心：1h 主趋势 + 15m 启动，4h 只用于置信度加权，不再做硬过滤
      - 方向：主要来自 1h EMA 结构
      - 置信度：0.6 * 1h_strength + 0.4 * 4h_strength（无信号时为 0）
      - 为提升交易频率：
          * 不再要求 4h/1h 强度必须 >= 某阈值
          * 只要方向一致 + 有突破 或 明显趋势坡度，即可给信号
    """
    df = df_1h.copy()
    ...
    # 1h 趋势坡度（近期趋势变化率）
    trend_slope = merged["trend_raw_1h"] - merged["trend_raw_1h"].shift(3)
    merged["trend_slope_1h"] = trend_slope

    # 方向判定：以 1h 为主，4h 仅用于置信度修正
    dir_1h = merged["trend_dir_1h"].fillna(0)
    dir_4h = merged["trend_dir_4h"].fillna(0)

    long_base = dir_1h > 0
    short_base = dir_1h < 0

    # 启动条件：
    #   - long: 1h 向上（dir_1h>0）并且 (15m 突破 or 趋势坡度向上)
    #   - short: 1h 向下并且 (15m 向下突破 or 趋势坡度向下)
    long_cond = long_base & (
        merged["breakout_up_1h"] | (merged["trend_slope_1h"] > 0)
    )
    short_cond = short_base & (
        merged["breakout_down_1h"] | (merged["trend_slope_1h"] < 0)
    )
    ...
    # 置信度：0.6 * 1h_strength + 0.4 * 4h_strength
    s1 = merged["trend_strength_1h"].fillna(0)
    s4 = merged["trend_strength_4h"].fillna(0)
    strength = 0.6 * s1 + 0.4 * s4
...
def build_signals_B(df_1h: pd.DataFrame,
    ...
    策略 B（V21_2）：
      - 完全独立于 4h/1h 趋势过滤
      - 方向来自因子方向：factor_dir_1h
      - 置信度来自因子强度：factor_strength_1h
      - 不再依赖 15m 突破（可以视为更稳健的“AI 策略”）
    """
    df = df_1h.copy()
    merged = df.join(multifactor_1h, how="left")

    dir_f = merged["factor_dir_1h"].fillna(0)
    strength_f = merged["factor_strength_1h"].fillna(0)

    merged["signal_dir_B"] = dir_f.astype(int)
...
def backtest_with_leverage(
    ...
    通用回测引擎：
      ...
      - 冷静期：最小持仓 bars
    不同策略通过参数调整行为：
      - A：趋势型（更大 ATR 倍数、更长持仓）
      - B：多因子（更小 ATR 倍数、更短持仓、更宽容触发）
    """
    df = df.copy()
    close = df["close"]
    high = df["high"]
...
def run_symbol_A_B(symbol: str, days: int) -> Tuple[BacktestResult, BacktestResult]:
    ...
    # 4h & 15m
    df_4h_raw = load_local_kline(symbol, "4h", days + 30)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=f"多周期趋势 + 多因子系统 ({VERSION_TAG}) - 策略A(趋势) & 策略B(多因子)"
    )
    p.add_argument(
        "--symbols",
        type=str,
