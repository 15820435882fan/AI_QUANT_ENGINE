# v31_rule_trend_system_v10.py
# V31 · 规则版 Teacher_V10 · 抽水机制版（修复复利和手续费问题）
# 修复：1) 真正的复利实现 2) 手续费计算修复 3) 杠杆调整生效
...
class ProfitWaterfall:
    ...
    def __init__(self, 
        ...
        self.trading_capital = initial_capital     # 用于交易的本金（复利增长）
    ...
    def update(self, current_trading_capital: float, timestamp: pd.Timestamp) -> float:
        """
        检查是否需要抽水，返回更新后的交易资金
        V10修复：实现真正的复利 - 抽水后剩余资金继续用于复利增长
        """
        # 计算相对于上一次抽水基准的增长百分比
        growth_pct = current_trading_capital / self.last_waterfall_level - 1.0
        
        # 如果增长达到阈值
        if growth_pct >= self.growth_threshold:
            # 计算增长金额
            growth_amount = current_trading_capital - self.last_waterfall_level
            ...
            # V10修复：抽水后的资金继续留在交易资金中实现复利
            # 注意：这里不再从交易资金中扣除，因为growth_amount已经是交易资金的增长
            ...
            record = {
                ...
                'new_trading_capital': current_trading_capital,  # 交易资金继续增长
                'new_profit_pool': self.profit_pool
            }
            self.history.append(record)
            
            # 更新抽水基准线（使用新的交易资金）
            self.last_waterfall_level = current_trading_capital
            
            # 输出日志
            print(f"[抽水] {timestamp}: 增长{growth_pct*100:.2f}% (+${growth_amount:.2f}), "
                  f"抽水${withdraw_amount:.2f}, 交易资金:${current_trading_capital:.2f}, "
                  f"利润池:${self.profit_pool:.2f}")
        
        return current_trading_capital
...
class V31V10Config:
    ...
    leverage: float = 3.0  # 默认3倍杠杆，可通过命令行调整到5-10倍
    ...
    max_drawdown_pct: float = 0.10  # 最大回撤强制平仓 10%
    ...
    # 极端波动过滤
    atr_spike_threshold: float = 2.0  # ATR突增倍数阈值
    
    # 复利模式
    compound_mode: bool = True  # 是否启用复利模式（默认启用）


class V31RuleTrendSystemV10Waterfall:
    def __init__(self, cfg: V31V10Config):
        self.cfg = cfg
    ...
    def _compute_sl_tp_notional(
        ...
        # V10修复：使用杠杆计算最大名义仓位时，需要正确使用杠杆参数
        # 风险预算反推名义仓位
        ...
        for ts, row in df.iloc[1:].iterrows():
            ...
            if side != "FLAT" and trade_type is not None:
                ...
                if exit_price is not None:
                    ...
                    # 记录复利收益率（相对于当前交易资金）
                    if self.cfg.compound_mode:
                        trade_return_pct = pnl / max(self.trading_capital - pnl, 1e-9)  # 使用交易前资金
                    ...
                    # 检查每日亏损限制
                    daily_loss_limit = self.waterfall.trading_capital * self.cfg.max_daily_loss_pct
                    if self.daily_pnl < -daily_loss_limit:
                        print(f"[风控] {ts}: 触发单日亏损限制，暂停交易")
                        # 这里可以添加暂停交易逻辑
                    
                    # 计算盈亏百分比（相对于当前交易资金）
                    pnl_pct = pnl / max(self.trading_capital - pnl, 1e-9)
                    ...
                    # === V10修复：抽水机制更新（实现真正的复利）===
                    if self.cfg.enable_waterfall:
                        # 使用平仓后的交易资金进行抽水检查
                        new_trading_capital = self.waterfall.update(
            ...
            # 记录资金曲线（包含浮动盈亏）
            # V10修复：资金曲线基于总价值和浮动盈亏
            current_equity = self.total_value + self.floating_pnl
            self.peak_equity = max(self.peak_equity, current_equity)
            
            # 检查最大回撤限制
            if self.peak_equity > 0:
                drawdown = (current_equity - self.peak_equity) / self.peak_equity
                if drawdown <= -self.cfg.max_drawdown_pct and side != "FLAT":
    ...
    def summary(self):
        ...
        print("=" * 80)
        print("V31 规则版 Teacher_V10 · 抽水机制版 回测结果（修复复利和手续费问题）")
        print("=" * 80)
        ...
        if self.cfg.enable_waterfall:
            ...
            print("抽水机制统计")
            print("-" * 40)
            print(f"当前交易资金: ${self.waterfall.trading_capital:,.2f}")
    ...
    def _print_detailed_statistics(self, annual_return: float, total_return: float):
        ...
        if len(returns) > 1:
            avg_return = np.mean(returns) * 252  # 年化（假设每天都有交易）
    ...
    def export_statistics_report(self, filepath: str):
        ...
        with pd.ExcelWriter(filepath, engine='openpyxl') as writer:
            ...
            summary_data = {
                '指标': [
                    '交易对', '测试天数', '杠杆倍数', '初始资金', '最终总资产',
                    '总收益率', '年化收益', '最大回撤', '交易次数', '胜率',
                    '平均盈利', '平均亏损', '实际盈亏比', '夏普比率'
                ],
                '数值': [
                    self.cfg.symbol, 
                    self.cfg.days, 
            ...
            if self.cfg.enable_waterfall and self.waterfall.history:
                waterfall_df = pd.DataFrame(self.waterfall.history)
                waterfall_df.to_excel(writer, sheet_name='抽水记录', index=False)
    ...
    def _plot_equity(self):
        ...
        # 资金曲线
        axes[0].plot(eq.index, eq.values, 'b-', linewidth=1.5)
        
        # 标记抽水事件
        if self.cfg.enable_waterfall and self.waterfall.history:
            for record in self.waterfall.history:
                axes[0].axvline(x=record['timestamp'], color='r', alpha=0.3, linestyle='--', linewidth=0.5)
        
        axes[0].set_title(f"V31 Teacher_V10 Waterfall System · {self.cfg.symbol} (Leverage:{self.cfg.leverage}x)")
        ...
        axes[1].fill_between(eq.index, drawdown, 0, color='red', alpha=0.3)
        axes[1].plot(eq.index, drawdown, 'r-', linewidth=1)
...
def parse_args():
    parser = argparse.ArgumentParser(description="V31 Teacher_V10 · 抽水机制版 回测（支持5-10倍杠杆）")
    parser.add_argument("--symbol", type=str, default="BTCUSDT", help="交易对")
    parser.add_argument("--days", type=int, default=365, help="回测天数")
    parser.add_argument("--leverage", type=float, default=3.0, help="杠杆倍数（支持5-10倍杠杆）")
    parser.add_argument("--initial-equity", type=float, default=10000.0, help="初始资金")
    parser.add_argument("--risk-per-trade", type=float, default=0.01, help="单笔风险比例")
    ...
    parser.add_argument("--export-report", type=str, default="", help="导出详细统计报告")
    parser.add_argument("--max-consecutive-losses", type=int, default=5, help="最大连续亏损次数")
    parser.add_argument("--max-daily-loss", type=float, default=0.05, help="单日最大亏损比例")
    parser.add_argument("--max-drawdown", type=float, default=0.10, help="最大回撤强制平仓")
    parser.add_argument("--no-compound", action="store_true", help="禁用复利模式")
    parser.add_argument("--atr-spike", type=float, default=2.0, help="ATR突增倍数阈值")
