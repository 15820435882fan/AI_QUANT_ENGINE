...
# V31 · 规则版 Teacher_V9 · 抽水机制版（修正版 v2）
# 修复：1) annual_return 未定义错误 2) 中文字体警告 3) 完善复利逻辑
...
class ProfitWaterfall:
    ...
    def update(self, current_trading_capital: float, timestamp: pd.Timestamp) -> float:
        """
        检查是否需要抽水，返回更新后的交易资金
        修正：只对已实现的交易资金增长抽水，不考虑浮动盈亏
        """
        # 计算相对于上一次抽水基准的增长百分比（只考虑交易资金）
        ...
        if growth_pct >= self.growth_threshold:
            ...
            # 更新抽水基准线（使用新的交易资金）
            self.last_waterfall_level = self.trading_capital
...
class V31V9Config:
    ...
    leverage: float = 100.0  # 在这里修改杠杆值！比如改成2.0或5.0
...
class V31RuleTrendSystemV9Waterfall:
        ...
        for ts, row in df.iloc[1:].iterrows():
            ...
            if side != "FLAT" and trade_type is not None:
                ...
                if exit_price is not None:
                    ...
                    # 记录复利收益率
                    trade_return_pct = pnl / max(self.waterfall.initial_capital, 1e-9)
                    ...
                    if self.cfg.enable_waterfall:
                        # 使用平仓后的交易资金进行抽水检查
                        self.trading_capital = self.waterfall.update(
    ...
    def _plot_equity(self):
        ...
        axes[0].set_title(f"V31 Teacher_V9 Waterfall System · {self.cfg.symbol} (Leverage:{self.cfg.leverage}x)")
