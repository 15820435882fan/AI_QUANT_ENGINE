# v31_rule_trend_system_v9_waterfall.py
# V31 · 规则版 Teacher_V9 · 抽水机制版
# 纯趋势跟随：只做顺势，不抓反转，只吃趋势中段
# 新增功能：利润抽水机制，每增长1%抽水10%放入利润池
...
class ProfitWaterfall:
    ...
    def __init__(self, 
                 ...
                 min_withdraw: float = 1.0,        # 最小抽水金额
                 enable_auto_withdraw: bool = False):  # 是否自动提取
        ...
        self.enable_auto_withdraw = enable_auto_withdraw
        
        # 抽水基准线（上一次抽水时的总资产）
        self.last_waterfall_level = initial_capital
        
        # 记录历史
        self.history: List[WaterfallRecord] = []
    ...
    def update(self, 
               ...
               timestamp: pd.Timestamp = None) -> float:
        """
        检查是否需要抽水，返回更新后的交易资金
        
        参数:
            current_total_value: 当前总资产（交易资金 + 浮动盈亏 + 利润池）
            timestamp: 当前时间
        ...
        """
        # 计算相对于上一次抽水基准的增长百分比
        growth_pct = current_total_value / self.last_waterfloor_level - 1.0
        
        # 如果增长达到阈值
        if growth_pct >= self.growth_threshold:
            # 计算增长金额
            growth_amount = current_total_value - self.last_waterfall_level
            ...
            # 记录抽水前的状态
            before_capital = self.trading_capital
            before_pool = self.profit_pool
            ...
            record = WaterfallRecord(
                ...
                after_capital=self.trading_capital,
                after_pool=self.profit_pool
            )
            self.history.append(record)
            
            # 更新统计
            self.stats['total_withdrawn'] += withdraw_amount
            self.stats['triggers_count'] += 1
            self.stats['max_growth_pct'] = max(self.stats['max_growth_pct'], growth_pct)
            self.stats['min_growth_pct'] = min(self.stats['min_growth_pct'], growth_pct)
            
            # 更新抽水基准线
            self.last_waterfall_level = self.get_total_value()
            
            # 输出日志
            print(f"[抽水机制] {timestamp}: "
                  f"增长{growth_pct*100:.2f}% (+${growth_amount:.2f}), "
                  f"抽水${withdraw_amount:.2f}, "
                  f"交易资金: ${self.trading_capital:.2f}, "
                  f"利润池: ${self.profit_pool:.2f}")
            
            # 可选：自动提取部分利润
            if self.enable_auto_withdraw and self.profit_pool > self.initial_capital * 0.1:
                # 如果利润池超过初始资金的10%，自动提取一半
                auto_withdraw = self.profit_pool * 0.5
                self.withdraw_profit(auto_withdraw)
        
        return self.trading_capital
    
    def withdraw_profit(self, amount: float = None) -> float:
        """
        从利润池提取资金
        
        参数:
            amount: 提取金额，None表示提取全部
            
        返回:
            float: 实际提取金额
        """
        if amount is None:
            amount = self.profit_pool
        
        # 确保不超过利润池余额
        withdraw_amount = min(amount, self.profit_pool)
        
        if withdraw_amount > 0:
            self.profit_pool -= withdraw_amount
            print(f"[利润提取] 提取${withdraw_amount:.2f}, "
                  f"剩余利润池: ${self.profit_pool:.2f}")
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        total_return_pct = (total_value / self.initial_capital - 1.0) * 100
        
        return {
            'initial_capital': self.initial_capital,
            'current_trading_capital': self.trading_capital,
            'profit_pool': self.profit_pool,
            'total_value': total_value,
            'total_return_pct': total_return_pct,
            'total_withdrawn': self.stats['total_withdrawn'],
            'waterfall_triggers': self.stats['triggers_count'],
            'avg_growth_per_trigger': self.stats['total_withdrawn'] / max(1, self.stats['triggers_count']),
            'withdraw_rate': self.withdraw_rate * 100,
            'growth_threshold': self.growth_threshold * 100,
            'last_waterfall_level': self.last_waterfall_level
        }
    ...
    def export_history(self, filepath: str):
        ...
        for record in self.history:
            rows.append({
                ...
                'before_capital': record.before_capital,
                'before_pool': record.before_pool,
                'growth_amount': record.growth_amount,
                'growth_pct': record.growth_pct * 100,
                'withdrawn': record.withdrawn,
                'remaining_growth': record.remaining_growth,
                'after_capital': record.after_capital,
                'after_pool': record.after_pool
            })
...
class V31V9Config:
    ...
    waterfall_min_withdraw: float = 10.0        # 最小抽水金额$10
    waterfall_auto_withdraw: bool = False       # 是否自动提取
...
class V31RuleTrendSystemV9Waterfall:
    def __init__(self, cfg: V31V9Config):
        ...
        self.waterfall = ProfitWaterfall(
            ...
            min_withdraw=cfg.waterfall_min_withdraw,
            enable_auto_withdraw=cfg.waterfall_auto_withdraw
        )
        
        # 资金状态
        self.trading_capital = cfg.initial_equity  # 实际交易资金
        self.total_value = cfg.initial_equity      # 总资产（交易资金+利润池）
        self.floating_pnl = 0.0                    # 浮动盈亏
        ...
        for ts, row in df.iloc[1:].iterrows():
            ...
            if self.cfg.enable_waterfall:
                ...
                if side != "FLAT":
                    ...
                    current_total_value = self.waterfall.get_total_value() + self.floating_pnl
                else:
                    self.floating_pnl = 0.0
                    current_total_value = self.waterfall.get_total_value()
                
                # 更新抽水机制
                self.trading_capital = self.waterfall.update(current_total_value, ts)
    ...
    def summary(self):
        ...
        # 抽水机制统计
        if self.cfg.enable_waterfall:
            waterfall_summary = self.waterfall.get_summary()
            print("\n===== 抽水机制统计 =====")
            print(f"交易资金: ${waterfall_summary['current_trading_capital']:,.2f}")
            print(f"利润池可提取: ${waterfall_summary['profit_pool']:,.2f}")
            print(f"总抽水金额: ${waterfall_summary['total_withdrawn']:,.2f}")
            print(f"抽水触发次数: {waterfall_summary['waterfall_triggers']}")
            print(f"平均每次抽水: ${waterfall_summary['avg_growth_per_trigger']:.2f}")
            print(f"抽水比例: {waterfall_summary['withdraw_rate']}%")
            print(f"增长阈值: {waterfall_summary['growth_threshold']}%")
            print("=======================")
    ...
    def export_waterfall_history(self, filepath: str):
        """导出抽水历史"""
        self.waterfall.export_history(filepath)
    ...
    def _plot_equity(self):
        ...
        # 绘制资金曲线
        plt.subplot(2, 1, 1)
        plt.plot(eq.index, eq.values, linewidth=1.5)
        plt.title(f"V31 Teacher_V9 资金曲线 · 抽水机制版（{self.cfg.symbol}, 5m）")
        plt.xlabel("Time")
        ...
        if self.cfg.enable_waterfall and self.waterfall.history:
            waterfall_times = [record.timestamp for record in self.waterfall.history]
            waterfall_values = [record.after_capital + record.after_pool for record in self.waterfall.history]
            plt.scatter(waterfall_times, waterfall_values, color='red', s=30, zorder=5, label='抽水事件')
            plt.legend()
        ...
        if self.cfg.enable_waterfall:
            # 交易资金 vs 利润池
            time_index = eq.index
            trading_capital_series = []
            profit_pool_series = []
            
            # 这里简化处理，实际应该记录每个时间点的资金构成
            # 为了演示，我们假设利润池随时间线性增长
            if self.waterfall.history:
                last_trading = self.waterfall.trading_capital
                last_pool = self.waterfall.profit_pool
                trading_capital_series = [last_trading] * len(time_index)
                profit_pool_series = [last_pool] * len(time_index)
                
                plt.bar(time_index, trading_capital_series, alpha=0.6, label='交易资金', color='blue')
                plt.bar(time_index, profit_pool_series, bottom=trading_capital_series, 
                       alpha=0.6, label='利润池', color='green')
                plt.xlabel("Time")
                plt.ylabel("Capital Breakdown (USDT)")
                plt.title("资金构成（交易资金 + 利润池）")
                plt.legend()
                plt.grid(True, alpha=0.3)
...
def parse_args():
    p = argparse.ArgumentParser(description="V31 Teacher_V9 · 抽水机制版 回测")
    p.add_argument("--symbol", type=str, default="BTCUSDT")
    p.add_argument("--days", type=int, default=365)
    p.add_argument("--no-plot", action="store_true")
    p.add_argument("--no-waterfall", action="store_true", help="禁用抽水机制")
    p.add_argument("--withdraw-rate", type=float, default=0.10, help="抽水比例，默认0.10（10%）")
    p.add_argument("--growth-threshold", type=float, default=0.01, help="增长触发阈值，默认0.01（1%）")
    p.add_argument("--export", type=str, default="", help="导出交易明细（.xlsx 或 .csv）")
    p.add_argument("--export-waterfall", type=str, default="", help="导出抽水历史")
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    if args.export_waterfall:
        system.export_waterfall_history(args.export_waterfall)