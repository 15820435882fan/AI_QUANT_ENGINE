# trading_env_v22_16.py
#
# V22_16: 多周期交易环境（强化趋势奖励 + 加强逆势惩罚 + 回撤惩罚 ×1.5）
# ------------------------------------------------------------------
# 变化要点：
# 1）保留 V22_15 的多周期特征与 Reward Shaping 4.0 框架
# 2）trend_reward ×3        → 放大强趋势区间的顺势收益
# 3）momentum_reward ×2     → 放大与短期动量一致的行为
# 4）anti_trend_penalty ↑   → 逆势在强趋势时被更重惩罚
# 5）drawdown_penalty ×1.5  → 对深回撤更敏感
# 6）reward clip: [-0.1,0.1] → [-0.2,0.2]，给 AI 更大学习空间
...
class TradingEnvV22MultiTFV16:
    """
    强化学习交易环境（单标的，多周期状态，Reward Shaping 4.0 强化版 + 数据切分）
    ...
    def __init__(
        ...
        logger.info(
            f"[EnvV22_16] segment={self.segment}, usable_len={len(self.df_state)}, "
            f"state_dim={self.state_dim}"
        )

    def _load_data_multitf(self) -> None:
        """加载多周期数据"""
        logger.info(f"[EnvV22_16] 加载多周期数据: {self.symbol}, days={self.days}")
    ...
    def step(self, action: int):
        ...
        # ===== Reward Shaping 4.0: 趋势强化版（V22_16 调参） =====
        ...
        # ★ V22_16: reward clip 区间扩大到 [-0.2, 0.2]
...
if __name__ == "__main__":
    # 简单双随机策略自检
    env = TradingEnvV22MultiTFV16(symbol="BTCUSDT", days=365, segment="train")
    obs = env.reset()
    logger.info(f"[EnvV22_16] 初始 obs 维度: {obs.shape}, 示例前 5 个: {obs[:5]}")
    total_r = 0.0
    steps = 0
    done = False
    while not done and steps < 300:
        ...
        steps += 1
    logger.info(
        f"[EnvV22_16] 随机策略测试结束: 步数={steps}, 总reward={total_r:.4f}, 最终权益={info.get('equity', None):.2f}"
    )
