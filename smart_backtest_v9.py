...
- 多因子 Alpha 引擎：
    ...
    * 波动因子 vol_level (ATR / close)
    * 策略共识因子 consensus_strength
    * Regime 识别 (strong_trend / weak_trend / range / low_vol / high_vol)
- Long/Short 各自 Alpha 打分 (alpha_long_score / alpha_short_score)
- 因子驱动开仓 + AI 仓位
- ATR 止损 / 止盈 + Trailing Stop
- 分币种风险参数（BTC / ETH 差异配置）
- 连续亏损冷静期
"""
...
def compute_alpha_factors(df: pd.DataFrame) -> pd.DataFrame:
    ...
    基于趋势、波动、策略共识等，计算：
      - vol_level: 归一化波动
      - regime: 市场结构标签
      - alpha_long_score / alpha_short_score: 多空方向 Alpha
    """
    d = df.copy()
    ...
    ensemble_raw = d["ensemble_raw"].values
    ensemble_dir = d["ensemble_dir"].values
    trend_long_ok = d["trend_long_ok"].values
    trend_short_ok = d["trend_short_ok"].values

    n = len(d)

    vol_level = np.zeros(n, dtype=float)
    regime = np.array(["unknown"] * n, dtype=object)
    alpha_long = np.zeros(n, dtype=float)
    alpha_short = np.zeros(n, dtype=float)
    ...
    for i in range(n):
        ...
        # 波动因子：ATR / close，映射到 [0,1]
        if np.isnan(a) or c <= 0:
            v = 0.0
        else:
            v = a / c  # 大约在 0~几百分比
        v = min(max(v * 200.0, 0.0), 1.0)  # 假设 0.5% ATR ~ 1.0

        vol_level[i] = v

        # Regime 识别
        if ts > 0.6 and 0.2 <= v <= 0.8:
            reg = "strong_trend"
        elif ts > 0.3 and 0.1 <= v <= 0.9:
            reg = "weak_trend"
        elif v < 0.1:
            reg = "low_vol"
        elif v > 0.9:
        ...
        # 策略共识因子
        consensus_strength = min(1.0, abs(eraw) * 2.0)

        # 波动适配因子：偏好中等波动
        vol_pref = 1.0 - abs(v - 0.5) * 1.5
        vol_pref = min(max(vol_pref, 0.0), 1.0)

        # 趋势方向偏好
        trend_long_factor = ts if trend_long_ok[i] else ts * 0.5
        trend_short_factor = ts if trend_short_ok[i] else ts * 0.5

        # Regime bonus：趋势类 regime 对趋势交易加分，range 对反转略加分
        reg_bonus_long = 0.0
        reg_bonus_short = 0.0
        if reg in ("strong_trend", "weak_trend"):
            reg_bonus_long = 0.15
            reg_bonus_short = 0.15
        elif reg == "range":
            # 区间更适合 BOLL / 反转，但这里先简单给轻微加成
            reg_bonus_long = 0.05
            reg_bonus_short = 0.05

        # 多空 Alpha 打分
        if edir > 0:
            # 策略偏多
            alpha_l = (
                0.4 * trend_long_factor
                + 0.4 * consensus_strength
                + 0.2 * vol_pref
                + reg_bonus_long
            )
            alpha_s = 0.2 * consensus_strength * (1 - trend_short_factor)
        elif edir < 0:
            # 策略偏空
            alpha_s = (
                0.4 * trend_short_factor
                + 0.4 * consensus_strength
                + 0.2 * vol_pref
                + reg_bonus_short
            )
            alpha_l = 0.2 * consensus_strength * (1 - trend_long_factor)
        else:
            # 策略没共识，以趋势为主
            alpha_l = 0.3 * trend_long_factor + 0.2 * vol_pref
            alpha_s = 0.3 * trend_short_factor + 0.2 * vol_pref

        alpha_long[i] = max(0.0, min(alpha_l, 1.5))  # 允许略大于1，后续会截断使用
        alpha_short[i] = max(0.0, min(alpha_s, 1.5))

    d["vol_level"] = vol_level
    d["regime"] = regime
    d["alpha_long_score"] = alpha_long
    d["alpha_short_score"] = alpha_short
...
class AdaptiveSignalEngine:
    ...
    - 动态策略权重 + 合成信号
    - 多因子 Alpha：trend / volatility / consensus / regime
    - 多空 Alpha 打分决定方向
    - 因子驱动动态仓位
    - ATR 止损 / 止盈 + Trailing Stop
    - 连续亏损冷静期
    ...
    def __init__(self):
        ...
        self.base_params = {
            ...
            "alpha_threshold": 0.55,   # Alpha 评分阈值（多空最大值）
        }
        # 分币种 override（ETH 更宽的止损、更低单笔风险）
    ...
    def run_symbol_backtest(
        ...
        for idx, row in d.iterrows():
            ...
            alpha_long_score = float(row.get("alpha_long_score", 0.0))
            alpha_short_score = float(row.get("alpha_short_score", 0.0))
            trend_strength = float(row.get("trend_strength", 0.0))
            consensus_strength = min(1.0, abs(float(row.get("ensemble_raw", 0.0)) * 2.0))
            ...
            if position == 0:
                ...
                # 选择多空方向：取 alpha 更大的一边
                alpha_long_c = max(0.0, min(alpha_long_score, 1.0))
                alpha_short_c = max(0.0, min(alpha_short_score, 1.0))

                best_alpha = max(alpha_long_c, alpha_short_c)
                if best_alpha < params["alpha_threshold"]:
                    # 市场不够“可预期”，不下手
                    continue

                long_signal = alpha_long_c >= alpha_short_c
                ...
                # 因子驱动风险：趋势越强 + 共识越高 → 风险略放大
                base_risk = params["base_risk_per_trade"]
                factor_boost = 0.4 * trend_strength + 0.6 * consensus_strength
                dyn_risk = base_risk * (0.5 + factor_boost)  # 大致在 [0.5x, 1.5x] 左右
                dyn_risk = max(dyn_risk, base_risk * 0.5)
                dyn_risk = min(dyn_risk, base_risk * 2.0)
...
def run_backtest(
    ...
    logger.info("🚀 SmartBacktest V9 启动")
...
def main():
    parser = argparse.ArgumentParser(description="SmartBacktest V9")
    parser.add_argument(
        "--symbols",
        type=str,
