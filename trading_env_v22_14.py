"""
trading_env_v22_14.py

V22_14: 多周期交易环境 + Train/Valid/Test 数据切分版
----------------------------------------------------
在 V22_5 的基础上增加：
...
class TradingEnvV22MultiTFV14:
    ...
    def __init__(
        ...
        logger.info(
            f"[EnvV22_14] segment={self.segment}, usable_len={len(self.df_state)}, "
            f"state_dim={self.state_dim}"
        )

    def _load_data_multitf(self) -> None:
        """加载多周期数据"""
        logger.info(f"[EnvV22_14] 加载多周期数据: {self.symbol}, days={self.days}")
    ...
    def step(self, action: int):
        ...
        if dd_ratio < 0.0:
            # 将超出 20% 的回撤部分线性惩罚，例如 30% 回撤 -> -0.005 * 0.1 = -0.0005
...
if __name__ == "__main__":
    # 简单双随机策略自检
    env = TradingEnvV22MultiTFV14(symbol="BTCUSDT", days=365, segment="train")
    obs = env.reset()
    logger.info(f"[EnvV22_14] 初始 obs 维度: {obs.shape}, 示例前 5 个: {obs[:5]}")
    total_r = 0.0
    steps = 0
    done = False
    while not done and steps < 300:
        ...
        steps += 1
    logger.info(
        f"[EnvV22_14] 随机策略测试结束: 步数={steps}, 总reward={total_r:.4f}, 最终权益={info.get('equity', None):.2f}"
    )