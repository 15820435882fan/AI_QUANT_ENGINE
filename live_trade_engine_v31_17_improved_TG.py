    ...
    def _call_with_retry(self, func, *args, **kwargs):
        ...
        for attempt in range(retries):
            try:
                ...
                last_err = e
                print(f"[Binance] 调用 {getattr(func, '__name__', func)} 失败，第 {attempt+1} 次重试: {e}")
                time.sleep(delay)
        print(f"[Binance] 多次重试后仍失败: {last_err}")
        raise last_err
    ...
    def create_market_order(
        ...
        try:
            ...
            try:
                ...
                avg_price = None
            if avg_price:
                print(f"[Binance] 下单成功: {symbol} {side} {amount} @≈{avg_price}")
            else:
                print(f"[Binance] 下单成功: {symbol} {side} {amount}")
            return order
        except Exception as e:
...
class LiveTradeEngineV31_15:
    def __init__(
        ...
        # 最近一次下单失败原因（只保留一条，便于排查）
        self.last_fail_reason: str = ""
        # 按币种统计 PnL（win: 盈利累积, loss: 亏损累积, sum: 净值）
    ...
    def _failsafe_force_close(self, sym: str, pos: LivePositionV31):
        ...
        if self.notifier:
            try:
                self.notifier.send(f"🛡️ Failsafe 启动 / Forced Close Try\n{sym} side={side_str} amount={amount}")
            except Exception:
                pass
        ...
        if order is not None:
            ...
            return False, "二次平仓仍失败，请人工检查交易所仓位。"
    ...
    def _handle_auto_exit(self, df_map: Dict[str, pd.DataFrame]):
        ...
        for sym, pos in list(self.positions.items()):
            ...
            if self.cfg_live.live_mode and self.binance is not None and pos.qty > 0:
                ...
                # 将一次完整平仓流程视为一次下单动作（包含初次 + Failsafe）
                ...
                if success:
                    ...
                    print(f"[ExitOrder] {sym} 平仓流程最终成功。")
                    if self.notifier:
                        try:
                            self.notifier.send(
                                f"📉 平仓成功 / Closed\n{sym} side={pos.side} qty={pos.qty}\n出场价/Exit≈{exit_price:.4f} PnL={pnl:.2f}"
                            )
                        except Exception:
                            pass
                    to_close.append(sym)
                    ...
                    self.last_fail_reason = (
                        f"{sym} 平仓最终失败: {fail_reason}" if fail_reason else f"{sym} 平仓最终失败（未知原因）"
                    )
                    print(f"[ExitOrder] {sym} 平仓流程最终失败，请人工检查。")
            else:
                # 非实盘模式 / 无 qty，仅在模型层平仓
                to_close.append(sym)
    ...
    def run_one_cycle(self):
                ...
                if order is not None:
                    ...
                    if self.notifier:
                        try:
                            _avg_price = None
                            try:
                                _info = (order or {}).get("info") if isinstance(order, dict) else None
                                _avg_price = (_info or {}).get("avgPrice") or (_info or {}).get("price")
                            except Exception:
                                _avg_price = None
                            self.notifier.send(
                                f"📈 开仓成功 / Opened\n{full_sym} {side} qty={amount}\n入场价/Entry≈{_avg_price or entry_price:.4f}\nSL={stop_price:.4f} TP={take_price:.4f} RR={rr_used:.2f}"
                            )
                        except Exception:
                            pass
                    ...
                    self.total_order_fail += 1
                    cycle_order_fail += 1
                    self.last_fail_reason = f"{full_sym} 开仓失败（交易所未返回订单）"
                    if self.notifier:
                        try:
                            self.notifier.send(f"❌ 开仓失败 / Open failed\n{full_sym} {side} qty={amount}")
                        except Exception:
                            pass
            else:
                # 模拟模式：只记账，不真实下单
        ...
        if self.last_fail_reason:
            print(f"最近一次下单失败原因: {self.last_fail_reason}")
    ...
    def handle_command(self, text: str) -> None:
        ...
        try:
            ...
            if cmd in ("/help", "help", "h"):
                self.notifier.send("可用命令 / Commands:\n/status 当前状态\n/pos 持仓\n/topk TopK评分\n/pnl 累计PnL\n/help 帮助")
                return
            if cmd in ("/status", "1"):
                equity = self.account.total_equity()
                trading = self.account.risk_capital()
                msg = (f"📊 状态 / Status\n"
                       f"Equity={equity:.2f}, RiskCap={trading:.2f}\n"
                       f"刷新次数/Refresh={self.refresh_count}, "
                       f"累计下单/Orders={self.total_order_sent}✔{self.total_order_success}/{self.total_order_fail}✖\n"
                       f"上次失败/LastFail={self.last_fail_reason or '无 None'}")
                self.notifier.send(msg)
                return
            if cmd in ("/pos", "2"):
                if not self.positions:
                    self.notifier.send("当前无持仓 / No open positions")
                    return
                lines = []
                for s, p in self.positions.items():
                    lines.append(f"{s} {p.side} qty={p.qty} entry={p.entry_price:.4f} SL={p.stop_price:.4f} TP={p.take_price:.4f}")
                self.notifier.send("📦 持仓 / Positions\n" + "\n".join(lines))
                return
            if cmd in ("/topk", "3"):
                if not self.last_topk_syms:
                    self.notifier.send("暂无 TopK 数据 / No TopK yet")
                else:
                    sc = ", ".join([f"{k}:{self.last_scores.get(k,0):.3f}" for k in self.last_topk_syms])
                    self.notifier.send(f"🏆 TopK: {self.last_topk_syms}\nScores: {sc}")
                return
            if cmd in ("/pnl", "4"):
                if not self.pnl_stats:
                    self.notifier.send("暂无 PnL 数据 / No PnL yet")
                    return
                lines = []
                for sym, st in self.pnl_stats.items():
                    if st["win"]==0 and st["loss"]==0 and st["sum"]==0:
                        continue
                    lines.append(f"{sym}: win={st['win']:.2f}, loss={st['loss']:.2f}, sum={st['sum']:.2f}")
                self.notifier.send("💰 PnL 统计 / PnL Stats\n" + ("\n".join(lines) if lines else "暂无"))
                return
            # 未知命令
            self.notifier.send("未知命令。发送 /help 查看帮助 / Unknown command. Send /help")
        except Exception:
            try:
                self.notifier.send("命令处理异常 / Command handling error")
            except Exception:
                pass
        ...
        while True:
            try:
                ...
                try:
                    ...
                    if notifier:
                        notifier.send(f"⚠️ 运行异常 / Loop error: {e}")
                except Exception:
                    pass
...
def main():
    ...
    engine = LiveTradeEngineV31_15(cfg_live, binance=binance_client, notifier=notifier)
    notifier.start_polling(engine.handle_command)
    engine.run_forever()
