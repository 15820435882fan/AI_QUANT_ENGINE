# v31_rule_trend_system_v8.py
# V31 · 规则版 Teacher_V8
# 「中等结构趋势跟随」：只做顺势，只吃中段
# - 4H + 1H 判断方向 & 趋势强度（regime_level >= 1 才允许交易）
# - 5m 执行周期：回调后右侧入场（EMA/MACD 金叉/死叉 + BOLL 中段）
# - 不做任何反转单，只做 TREND
# - 动态 RR：强趋势≈4，中趋势≈3，弱趋势不参与
# - 单笔风险 1%，导出交易明细到 Excel/CSV
...
class Trade:
    ...
    macro_dir: float
    trend_dir: float
    regime_level: int  # 2=强趋势,1=中趋势,-1=冲突
    macd_hist_5m: float
    boll_pos_5m: float
    atr_used: float
    rr_used: float
...
class V31V8Config:
    ...
    max_bars_medium: int = 72   # ≈6 小时

    fee_rate: float = 0.0004
    slippage: float = 0.0002


class V31RuleTrendSystemV8:
    def __init__(self, cfg: V31V8Config):
        self.cfg = cfg
    ...
    def _calc_indicators(self):
        ...
        # regime_level: 2=强趋势,1=中趋势,-1=冲突/无方向
        regime_level = np.full(len(df5), -1, dtype=int)
    ...
    def _entry_signal(
        ...
        # 只做中/强趋势，且 4H 与 1H 同向
        if regime_level < 1 or macro_dir == 0 or trend_dir == 0:
            return "FLAT", None
        ...
        ema_fast_5_prev = prev_row["ema_fast_5m"]
        ema_slow_5_prev = prev_row["ema_slow_5m"]

        # -------- 多头趋势：只吃中段 --------
        if macro_dir > 0 and trend_dir > 0 and macd_15 > 0:
            # 价格在 BOLL 中上区间（不抄底，不追顶）
            cond_price = (boll_pos >= 0.35) and (boll_pos <= 0.8)
            # 右侧信号：EMA 金叉 或 MACD 由负转正
            cond_trigger = (
                (ema_fast_5_prev <= ema_slow_5_prev and ema_fast_5 > ema_slow_5)
                or (macd_5_prev <= 0 and macd_5 > 0)
            )
            if cond_price and cond_trigger and is_15m_bar:
                return "LONG", "TREND"

        # -------- 空头趋势：对称逻辑 --------
        if macro_dir < 0 and trend_dir < 0 and macd_15 < 0:
            cond_price = (boll_pos <= 0.65) and (boll_pos >= 0.2)
            cond_trigger = (
                (ema_fast_5_prev >= ema_slow_5_prev and ema_fast_5 < ema_slow_5)
                or (macd_5_prev >= 0 and macd_5 < 0)
            )
    ...
    def _compute_sl_tp_notional(
        ...
        # 与 4H 同向 → 不减仓；逆向 → 减半（理论上不会出现逆向，这里只是保险）
    ...
    def run_backtest(self, plot: bool = False):
        ...
        for ts, row in df.iloc[1:].iterrows():
            ...
            if side != "FLAT" and trade_type is not None:
                ...
                # 趋势反转/无趋势时，强制平仓（再交给下一段趋势决定）
                if exit_price is None and regime_level_now < 1:
                    exit_price = price_close
    ...
    def summary(self):
        ...
        print("===== V31 规则版 Teacher_V8 · 中等结构趋势跟随 回测结果 =====")
    ...
    def _plot_equity(self):
        ...
        plt.title(f"V31 Teacher_V8 资金曲线 · 中等结构趋势跟随（{self.cfg.symbol}, 5m）")
        plt.xlabel("Time")
...
def parse_args():
    p = argparse.ArgumentParser(description="V31 Teacher_V8 · 中等结构趋势跟随 回测")
...
if __name__ == "__main__":
    args = parse_args()
    cfg = V31V8Config(symbol=args.symbol.upper(), days=args.days)
    system = V31RuleTrendSystemV8(cfg)
