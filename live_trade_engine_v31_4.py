...
V31_4 · 多币轮动 · Binance 永续合约 实盘引擎（实时循环版）
--------------------------------------------------------
设计目标：
- 使用 V31_1 核心策略内核（v31_core_v31_1）：统一指标、信号、仓位和抽水模型
- 多币种 TopK 轮动，支持 Futures 合约
- 实时从 Binance 获取 5m K 线（不再依赖本地 CSV）
- 每 10 秒刷新一次行情与信号（可通过参数调整）
- 连接 Binance 读取真实账户余额
- 目前版本：
    * ✅ 实时选币 + 计算信号 + 真实余额展示
    * ✅ 在 --live 且 config.enable_trading=True 时才真正发起订单
    * ⚠ 还未实现自动平仓/止损/止盈管理（仅下单，不管理仓位）
"""
...
class BinanceFuturesV31:
    def __init__(self, cfg: dict):
        ...
        binance:
          ...
          options:
            defaultType: future
          enable_trading: true   # 只有为 True 才允许真正下单
        """
        ...
        self.enable_trading = bool(ex_cfg.get("enable_trading", False))
    ...
    # ---- 行情 ----
    def fetch_ohlcv_5m(self, symbol: str, lookback_minutes: int = 60 * 24):
        """
        ...
        try:
            ...
            print(f"[Binance] fetch_ohlcv 失败: {symbol}, err={e}")
            return None
    ...
    def fetch_futures_equity_usdt(self) -> Optional[float]:
        """
        尝试从 futures 账户中获取 USDT 总权益。
        不同账户结构下 ccxt 返回结构略有差异，这里做一点兼容处理。
        """
        try:
            balance = self.ex.fetch_balance()
        except Exception as e:
            print(f"[Binance] 获取账户余额失败: {e}")
            return None
        ...
        # 打印一次原始结构，方便调试
        if eq is None:
            print("[Binance] 警告：未能从 fetch_balance 中解析出 USDT 总权益，原始结构：")
            print(balance)
...
class LiveEngineConfig:
    ...
    lookback_minutes: int = 60 * 24  # 每次拉取多少分钟的历史数据用来计算指标
...
class LiveTradeEngineV31_4:
    ...
    def _calc_trend_score(self, df5: pd.DataFrame) -> float:
        ...
        recent = df5.iloc[-12 * 24 :]  # 最近约 24 小时
        if recent.empty:
            return 0.0
    ...
    def run_one_cycle(self):
        print(
            f"\n==== V31_4 周期刷新 · symbols={self.cfg_live.symbols}, topk={self.cfg_live.topk}, "
            f"live={self.cfg_live.live_mode}, refresh={self.cfg_live.refresh_seconds}s ===="
        )
        ...
        for sym in self.cfg_live.symbols:
            ...
            if self.binance is not None:
                df_raw = self.binance.fetch_ohlcv_5m(full_sym, lookback_minutes=self.cfg_live.lookback_minutes)
        ...
        print(
            "[TopK] 当前最强: "
            f"{topk_syms}, scores={{{"
            + ", ".join([f"{k}: {v:.6f}" for k, v in score_map.items()])
            + "}}"
        )
        ...
        for full_sym in topk_syms:
            ...
            qty = notional / entry_price  # 名义仓位对应的合约张数（未除杠杆）
    ...
    def run_forever(self):
        print(
            "==== 启动 V31_4 多币轮动·Binance Futures 实盘引擎（实时循环） ===="
        )
        print(
            f"symbols = {self.cfg_live.symbols}, timeframe=5m, topk={self.cfg_live.topk}"
        )
        print(
...
# ===========================
def parse_args():
    p = argparse.ArgumentParser(description="V31_4 多币轮动 · Binance Futures 实时引擎")
    p.add_argument(
        "--symbols",
        type=str,
    ...
    p.add_argument(
        "--live",
        action="store_true",
        help="启用实盘模式（连接 Binance Futures），仍需在 config_live_v31.yaml 中设置 enable_trading 才会真实下单",
    )
    return p.parse_args()
