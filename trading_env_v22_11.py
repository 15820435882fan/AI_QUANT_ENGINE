"""
trading_env_v22_11.py

V22_11: 多周期交易环境 + Train/Valid/Test 数据切分版
----------------------------------------------------
在 V22_5 的基础上增加：
...
class TradingEnvV22MultiTFV11:
    ...
    def __init__(
        ...
        logger.info(
            f"[EnvV22_11] segment={self.segment}, usable_len={len(self.df_state)}, "
    ...
    def _load_data_multitf(self) -> None:
        logger.info(f"[EnvV22_11] 加载多周期数据: {self.symbol}, days={self.days}")
...
if __name__ == "__main__":
    # 简单双随机策略自检
    env = TradingEnvV22MultiTFV11(symbol="BTCUSDT", days=365, segment="train")
    obs = env.reset()
    logger.info(f"[EnvV22_11] 初始 obs 维度: {obs.shape}, 示例前 5 个: {obs[:5]}")
    total_r = 0.0
    steps = 0
    done = False
    while not done and steps < 300:
        ...
        steps += 1
    logger.info(
        f"[EnvV22_11] 随机策略测试结束: 步数={steps}, 总reward={total_r:.4f}, 最终权益={info.get('equity', None):.2f}"
    )