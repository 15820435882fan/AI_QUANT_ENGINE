...
def load_local_kline(symbol: str, interval: str, days: int):
    ...
    if "timestamp" not in df.columns:
        logging.error(f"❌ CSV 缺少 timestamp: {path}")
        return None
    ...
    logging.info(f"📥 [LocalDataEngine] 载入本地数据: {symbol} {interval}, 天数={days}, 行数={len(df)}")
    return df
# ==================================
...
def compute_structure_factors(df_ltf: pd.DataFrame, df_mtf: pd.DataFrame, valid_bis):
    ...
    if total_len <= 0:
        ...
        # -1 ~ 1, 向上为正，向下为负
        struct_bias = (up_len - down_len) / total_len
        # 强度取绝对值，线性放大后截断
        struct_score = min(1.0, abs(struct_bias) * 2.0)
...
def generate_structure_signals(df_ltf: pd.DataFrame, valid_bis):
    ...
    核心逻辑：
    1. 先为每一笔动态计算 high / low（从 df_ltf 截面中取）
    2. 三笔组合：
       - up, down, up & 高点依次抬高 & 回调不破前低 → 做多信号
       - down, up, down & 低点依次降低 & 反弹不过前高 → 做空信号
    3. 单笔延伸：
       - 当某一笔涨跌幅绝对值够大（>2%），视为趋势延伸，在笔结束处给方向信号
    """
    long_signals = set()
    short_signals = set()
    ...
    # --- 为每一笔补齐 high / low 属性 ---
    for bi in valid_bis:
        if not hasattr(bi, "high"):
            bi.high = highs[bi.start_index : bi.end_index + 1].max()
        if not hasattr(bi, "low"):
            bi.low = lows[bi.start_index : bi.end_index + 1].min()
...
def run_symbol(symbol, df_ltf, df_mtf, df_htf, capital=10000.0):
    ...
    bis = detect_bis(fractals, highs, lows, closes)
    valid_bis = filter_valid_bis(bis, min_bars=7, min_move_pct=0.003)

    # 3) 结构方向因子
    struct = compute_structure_factor(valid_bis)
    ma_bias = compute_ma_bias(df_ltf)
    final_bias = 0.6 * struct + 0.4 * ma_bias

    regime = (
        "trend" if final_bias >= 0.66
        else "range" if final_bias <= 0.33
        else "mixed"
    )

    logging.info(
        f"📐 {symbol} bi_total={len(bis)}, bi_valid={len(valid_bis)}, "
        f"struct={struct:.2f}, ma={ma_bias:.2f}, final={final_bias:.2f}, regime={regime}"
    )
    ...
    for i in range(len(df_ltf)):
        ...
        if position == 0:
            ...
            # 结构做多信号
            if i in long_signals and final_bias > 0.40:
                position = 1
                entry_price = close[i]
                continue

            # 结构做空信号
            if i in short_signals and final_bias < 0.60:
                position = -1
                entry_price = close[i]
                continue
...
def main():
    ...
    logging.info("🚀 SmartBacktest V16_1 启动")
