# -*- coding: utf-8 -*-
"""
live_trade_engine_v31_12.py

V31_12 · 多币轮动 · Binance USDT-M 永续合约 实盘引擎
在 V31_10 基础上加强：

1）自动 + 手动双保险检测账户持仓模式（单向 / 对冲）：
    - 若 config_live_v31.yaml 中显式写了 binance.hedge_mode，则优先使用该值
    - 否则：
        使用 fetch_balance(params={"type": "future"}) 获取账户信息，
        从 info.positions[*].positionSide 中判断：
            若出现 "LONG"/"SHORT" → hedge_mode=True（对冲模式）
            若仅有 "BOTH" 或未找到 → hedge_mode=False（单向持仓）

2）在 hedge_mode=True 时：
    - 开多：params["positionSide"] = "LONG"
    - 开空：params["positionSide"] = "SHORT"
    - 平仓：params["positionSide"] 与原方向一致 + reduceOnly=True

3）在 hedge_mode=False 时：
    - 不传 positionSide，仅使用 side="buy"/"sell" + reduceOnly 控制

其他逻辑与 V31_10 完全一致：
    - 账户资金只由策略自身的 PnL 驱动（不再用实盘权益差额）
    - 模型资金与实盘权益解耦，实盘权益仅作为参考打印
    - 指标 / 入场 / 平仓逻辑尽量 1:1 贴近 V31_1 回测
"""
...
class LiveTradeEngineV31_12:
    ...
    def run_one_cycle(self):
        ...
        print(
            f"\n==== V31_12 周期刷新 · symbols={self.cfg_live.symbols}, "
    ...
    def run_forever(self):
        print("==== 启动 V31_12 多币轮动·Binance USDT-M Futures 实盘引擎（实时循环） ====")
        print(
...
# ===========================
def parse_args():
    p = argparse.ArgumentParser(description="V31_12 多币轮动 · Binance USDT-M Futures 实时引擎")
    p.add_argument(
        "--symbols",
        type=str,
...
def main():
    ...
    engine = LiveTradeEngineV31_12(cfg_live, binance=binance_client)
    engine.run_forever()
