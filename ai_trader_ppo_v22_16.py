# ai_trader_ppo_v22_16.py
#
# V22_16: AI Trader · PPO 训练器（rollout=8192 强探索 + 强趋势版）
# ------------------------------------------------------------------
# 用法示例：
#   python ai_trader_ppo_v22_16.py --symbol BTCUSDT --days 365 --total-steps 150000
#
# 仅评估模式：
#   python ai_trader_ppo_v22_16.py --symbol BTCUSDT --days 365 --mode eval \
#       --model-path models/ai_trader_ppo_V22_16_BTCUSDT_best_valid.pt --eval-split test
...
from trading_env_v22_16 import TradingEnvV22MultiTFV16
...
class PPOConfig:
    ...
    lr: float = 3e-4
    train_epochs: int = 10
    batch_size: int = 8192        # ★ V22_16: B 档 rollout 8192
    minibatch_size: int = 256
    entropy_coef: float = 0.15    # ★ V22_16: 探索系数从 0.10 提到 0.15
    value_coef: float = 0.5
    max_grad_norm: float = 0.5
    valid_eval_interval: int = 1      # 每多少个训练大轮做一次验证
    early_stop_patience: int = 8      # 目前不再用于提前停止，只做占位
...
def train_ppo(
    ...
    # 三段环境
    env_train = TradingEnvV22MultiTFV16(symbol=symbol, days=days, segment="train")
    env_valid = TradingEnvV22MultiTFV16(symbol=symbol, days=days, segment="valid")
    env_test = TradingEnvV22MultiTFV16(symbol=symbol, days=days, segment="test")

    state_dim = env_train.state_dim
    action_dim = env_train.action_space_n

    logger.info(f"[PPO {VERSION_TAG}] 训练配置: symbol={symbol}, days={days}, total_steps={total_steps}")
    logger.info(f"[PPO {VERSION_TAG}] 状态维度={state_dim}, 动作维度={action_dim}, 设备={DEVICE}")
    ...
    while steps_collected < total_steps:
        ...
        logger.info(
            f"[PPO {VERSION_TAG}] Round={train_round}, 已采样步数: {steps_collected}/{total_steps}, "
        ...
        if train_round % config.valid_eval_interval == 0:
            ...
            if valid_equity > best_valid_equity:
                ...
                logger.info(f"[VALID {VERSION_TAG}] 新的最佳验证模型已保存: equity={best_valid_equity:.2f}")
            else:
                logger.info(f"[VALID {VERSION_TAG}] 验证未提升，继续训练（不早停，仅更新 best_valid）")
    ...
    if os.path.exists(best_valid_path):
        ...
        logger.info(f"[PPO {VERSION_TAG}] 加载最佳验证模型: {best_valid_path}")
    else:
        logger.warning(f"[PPO {VERSION_TAG}] 未找到 best_valid 模型，使用当前最新参数。")

    env_test = TradingEnvV22MultiTFV16(symbol=symbol, days=days, segment="test")
    test_equity, test_reward, test_steps = evaluate_policy_on_env(policy, env_test, max_steps=1000)
    logger.info(
        f"[TEST {VERSION_TAG}] 最终测试结果: equity={test_equity:.2f}, total_reward={test_reward:.4f}, steps={test_steps}"
    )
...
def evaluate_model(
    ...
    env = TradingEnvV22MultiTFV16(symbol=symbol, days=days, segment=eval_split)
    logger.info(f"[EVAL {VERSION_TAG}] 开始评估模型: {model_path} | split={eval_split}")

    final_equity, total_reward, steps = evaluate_policy_on_env(policy, env, max_steps=max_steps)
    logger.info(
        f"[EVAL {VERSION_TAG}] 评估结束: split={eval_split}, 步数={steps}, "
...
def parse_args():
    ...
    p.add_argument(
        "--total-steps",
        type=int,
        default=150000,   # ★ V22_16: 默认 150k 配合 rollout=8192
        help="训练阶段需要采样的总步数",
    )
    p.add_argument(
        "--mode",
...
def main():
    ...
    if args.mode == "train":
        ...
        logger.info(f"[MAIN {VERSION_TAG}] 训练结束，最佳验证模型路径: {best_valid_path}")
    else:
