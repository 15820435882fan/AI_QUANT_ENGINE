"""
ai_trader_ppo_v22_7.py

V22_7: AI Trader · PPO 训练器（Train / Valid / Test 三阶段）
---------------------------------------------------------

用法示例：
    python ai_trader_ppo_v22_7.py --symbol BTCUSDT --days 365 --total-steps 100000

仅评估模式：
    python ai_trader_ppo_v22_7.py --symbol BTCUSDT --days 365 --mode eval \
        --model-path models/ai_trader_ppo_V22_7_BTCUSDT_best_valid.pt --eval-split test
"""
...
from trading_env_v22_7 import TradingEnvV22MultiTFV7
...
class PPOConfig:
    ...
    early_stop_patience: int = 4      # 连续多少轮验证不提升则提前停止
...
def train_ppo(
    ...
    config: PPOConfig,
    save_prefix: str,
    split_mode: str = "recent_6_1_5",
) -> str:
    """训练 PPO 模型，返回 best_valid 模型路径。"""
    # 三段环境（V22_7 支持 split_mode）
    env_train = TradingEnvV22MultiTFV7(symbol=symbol, days=days, segment="train", split_mode=split_mode)
    env_valid = TradingEnvV22MultiTFV7(symbol=symbol, days=days, segment="valid", split_mode=split_mode)
    env_test = TradingEnvV22MultiTFV7(symbol=symbol, days=days, segment="test", split_mode=split_mode)
    ...
    while steps_collected < total_steps:
        ...
        if train_round % config.valid_eval_interval == 0:
            ...
            if valid_equity > best_valid_equity:
                ...
                logger.info(f"[VALID] 新的最佳验证模型已保存: equity={best_valid_equity:.2f}")
            else:
                no_improve_rounds += 1
                logger.info(
                    f"[VALID] 验证未提升, 连续次数={no_improve_rounds} / {config.early_stop_patience}"
                )
    ...
    env_test = TradingEnvV22MultiTFV7(symbol=symbol, days=days, segment="test")
...
def evaluate_model(
    ...
    env = TradingEnvV22MultiTFV7(symbol=symbol, days=days, segment=eval_split, split_mode=split_mode)
...
def parse_args():
    ...
    p.add_argument(
        "--split-mode",
        type=str,
        default="recent_6_1_5",
        choices=["ratio", "recent_6_1_5"],
        help="数据切分模式：ratio=按比例, recent_6_1_5=最近12个月按6/1/5切分",
    )
    return p.parse_args()
