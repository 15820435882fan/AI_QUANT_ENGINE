"""
smart_backtest_v19_1.py

V19_1: 多级别缠论 + AI 评分 + 结构化风控 回测引擎（优化可跑版）

说明：
- 依赖 local_data_engine.load_local_kline() 读取本地 K 线
- 默认使用多周期：4h / 1h / 15m / 5m
- 在 V19 基础上做了以下优化：
  1）放宽趋势触发条件（MA 交叉加容忍度）
  2）放宽 AI 评分阈值（min_score 从 0.6 降到 0.45）
  3）放宽 RR 要求（从 1.8 降到 1.2）
  4）去掉方向去重约束，允许连续同方向信号
"""
...
class ChanSignal:
    ...
    price: float
    kind: str                # trend_break / future: third_buy / third_sell 等
...
def compute_trend_strength(df: pd.DataFrame, window_short: int = 20, window_long: int = 60) -> float:
    ...
    if len(short_ma_series.dropna()) < 5 or pd.isna(short_ma) or pd.isna(long_ma):
        return 0.0

    slope = short_ma_series.dropna().iloc[-1] - short_ma_series.dropna().iloc[-5]
...
def generate_signals_v19_1(
    ctx: MultiLevelContext,
    exec_interval: str = "5m",
    rr_target: float = 1.2,       # RR 更宽松
    min_score: float = 0.45,      # 评分阈值更宽松
) -> List[ChanSignal]:
    """
    非完美缠论版，但可跑、可迭代：
    - 在执行级别上使用更敏感的 MA 趋势切换逻辑
    - 结合多级别 AI 评分进行过滤
    - 放宽 RR、评分阈值，让策略先“活起来”
    """
    ...
    for i in range(long_win + 5, len(df_exec) - 5):
        ...
        if short_ma.iloc[i] > long_ma.iloc[i] * 0.999:
            dir_now = "long"
        elif short_ma.iloc[i] < long_ma.iloc[i] * 1.001:
            dir_now = "short"
    ...
    logging.info(f"[{ctx.symbol}] 生成信号数量（V19_1 优化后）: {len(signals)}")
    return signals
...
def run_symbol_v19_1(
    ...
    logging.info(f"========== 开始回测 V19_1: {symbol} ==========")
    ctx = build_multilevel_context(symbol, intervals, days)
    df_exec = ctx.interval_dfs["5m"]

    signals = generate_signals_v19_1(ctx, exec_interval="5m")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="SmartBacktest V19_1 - 多级别缠论 + AI 评分 回测引擎（优化可跑版）"
    )
    p.add_argument(
        "--symbols",
        type=str,
...
def main():
    ...
    for sym in syms:
        try:
            res = run_symbol_v19_1(sym, args.days)
