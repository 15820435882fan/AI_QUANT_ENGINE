...
V22_12: AI Trader · PPO 训练器（Train / Valid / Test 三阶段）
---------------------------------------------------------
...
仅评估模式：
    python ai_trader_ppo_v22_12.py --symbol BTCUSDT --days 365 --mode eval \
        --model-path models/ai_trader_ppo_V22_12_BTCUSDT_best_valid.pt --eval-split test
"""
...
from trading_env_v22_12 import TradingEnvV22MultiTFV12
...
def train_ppo(
    ...
    # 三段环境
    env_train = TradingEnvV22MultiTFV12(symbol=symbol, days=days, segment="train")
    env_valid = TradingEnvV22MultiTFV12(symbol=symbol, days=days, segment="valid")
    env_test = TradingEnvV22MultiTFV12(symbol=symbol, days=days, segment="test")
...
def evaluate_model(
    ...
    env = TradingEnvV22MultiTFV12(symbol=symbol, days=days, segment=eval_split)  # 修复类名
